...
def forwardModelKinetics(
    ...
    Ea = kinetics[
        0
    ]  # Moles aren't passed into this function, so first entry of kinetics is Ea
    ...
    if geometry == "spherical":
        ...
        ) ** (1 / 2) - (3 / (torch.pi**2)) * (torch.pi**2 * Dtaa[Bt <= 1.401])
...
def calc_lnd0aa(sumf_MDD, diffti, geometry, extra_steps, added_steps):
    ...
    if geometry == "spherical":
        ...
        DR2_a[1:] = (1 / (torch.pi**2 * diffti)) * (
            -torch.pi**2 / 3 * diffFi
            - 2
            * torch.pi
            * (
                torch.sqrt(1 - (torch.pi / 3) * Fi[1:])
                - torch.sqrt(1 - (torch.pi / 3) * Fi[0:-1])
            )
        )
        DR2_b[0] = (
            -1 / (torch.pi**2 * cumtsec[0]) * torch.log((1 - Fi[0]) * (torch.pi**2 / 6))
        )
        DR2_b[1:] = (
            -1 / (torch.pi**2 * diffti) * torch.log((1 - Fi[1:]) / (1 - Fi[0:-1]))
        )
        ...
        DR2_a[1:] = ((((sumf_MDD[1:] ** 2) - (sumf_MDD[0:-1]) ** 2)) * torch.pi) / (
            4 * diffti[1:]
        )
        DR2_b[1:] = (4 / ((torch.pi**2) * diffti[1:])) * torch.log(
