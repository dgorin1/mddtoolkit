...
def forwardModelKinetics(
    ...
    if num_vectors == 1:
        ...
        fracs = torch.tile(
            torch.concat(
                (fracstemp, 1 - torch.sum(fracstemp, axis=0, keepdim=True)), axis=0
            ).T,
            (len(TC), 1),
        ...
        if ndom > 1:
            ...
            cumtsec = torch.tile(
                torch.reshape(torch.cumsum(tsec[:, 1], dim=0), (-1, 1)),
                (1, Ea.shape[1]),
        ...
        fracs = (
            torch.cat((fracstemp, 1 - torch.sum(fracstemp, axis=0, keepdim=True)))
            .unsqueeze(0)
            .expand(len(TC), -1, -1)
        )
        ...
        if ndom > 1:
            cumtsec = (
                torch.cumsum(tsec, dim=0).unsqueeze(-1).repeat(1, ndom)
...
def forward_model_kinetics_no_extra_heating(
    ...
    if num_vectors == 1:
        lnD0aa = torch.tile(
            kinetics[0:ndom].permute(*torch.arange(kinetics[0:ndom].ndim - 1, -1, -1)),
            (len(TC), 1),
        )  # Do this for LnD0aa
        fracstemp = kinetics[
            ndom:
...
def calc_lnd0aa(sumf_MDD, diffti, geometry, extra_steps, added_steps):
    ...
    if geometry == "spherical":
        ...
        Daa_MDD_b[0] = (1 / ((torch.pi**2) * diffti[0])) * (
            (2 * torch.pi)
