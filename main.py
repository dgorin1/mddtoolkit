...
omit_value_indices = [33,34,35,36,37,38,39,40,41] # Values you want to be ignored in your fit
misfit_stat_list = ["chisq","lnd0aa_chisq","percent_frac","l1_frac_cum","l1_frac","l1_moles","l2_moles","l2_frac","lnd0aa"] # This is a list of all the options. The user should just pick one.
...
for misfit_stat in misfit_stat_list:
    ...
    while i < max_domains_to_model+1 and misfit_val < prev_misfit:
        ...
        params, misfit_val = diffEV_multiples(objective,dataset,iteration_repeats,domains_to_model,Ea_bounds = Ea_bounds, lnd0aa_bounds = lnd0aa_bounds, max_iters = max_iters)
