...
def calc_arrhenius(
    kinetics, tsec, TC, geometry, extra_steps=True, added_steps: int = 0
):
    """
    ...
    Args:
        ...
        geometry (str): A string indicating the geometry of the sample. Either "spherical" or "plane sheet".
        extra_steps (bool, optional): A boolean indicating whether or not to add two extra steps to the calculation. Defaults to True.
        added_steps (int, optional): The number of extra steps to add to the calculation. Defaults to 0.
    """
    ...
    fracstemp = temp[
        ndom:
    ]  # Grab fracs that were input (one will be missing because it is pre-determined by the others)
    fracs = torch.tile(
        torch.concat(
            (fracstemp, 1 - torch.sum(fracstemp, axis=0, keepdim=True)), axis=-1
        ),
        (len(TC), 1),
    )  # Add the last frac as 1-sum(other fracs)
    Ea = torch.tile(kinetics[0], (len(TC), ndom))  # Do for Ea

    # Put time and cumulative time in the correct shape
    if ndom > 1:
        tsec = torch.tile(
            torch.reshape(tsec, (-1, 1)), (1, Ea.shape[1])
        )  # This is a complicated-looking way of getting tsec into a numdom x numstep matrix for multiplication
        cumtsec = torch.tile(
            torch.reshape(torch.cumsum(tsec[:, 1], dim=0), (-1, 1)), (1, Ea.shape[1])
        )  # Same as above, but for cumtsec

        # Convert TC to TK and put in correct shape for quick computation
        TK = torch.tile(
            torch.reshape((TC + 273.15), (-1, 1)), (1, Ea.shape[1])
        )  # This is a complicated-looking way of turning TC from a 1-d array to a 2d array and making two column copies of it
    ...
    if geometry == "spherical":
        ...
        f[Bt > 0.0091] = (6 / (torch.pi ** (3 / 2))) * torch.sqrt(
            (torch.pi**2) * Dtaa[Bt > 0.0091]
        ) - (3 / (torch.pi**2)) * ((torch.pi**2) * Dtaa[Bt > 0.0091])
        f[Bt > 1.8] = 1 - (6 / (torch.pi**2)) * torch.exp(
            -(torch.pi**2) * Dtaa[Bt > 1.8]
        )
    elif geometry == "plane sheet":
        f = (2 / np.sqrt(math.pi)) * np.sqrt((Dtaa))
        f[f > 0.6] = 1 - (8 / (math.pi**2)) * np.exp(
            -math.pi**2 * Dtaa[f > 0.6] / 4
        )
    ...
    if geometry == "spherical":
        Daa_MDD_a[0] = (sumf_MDD[0] ** 2 - 0.0**2) * torch.pi / (36 * (diffti[0]))

        # Equation 5a for all other steps

        Daa_MDD_a[1:] = (
            ((sumf_MDD[1:]) ** 2 - (sumf_MDD[0:-1]) ** 2)
            * math.pi
            / (36 * (diffti[1:]))
        )

        # Fechtig and Kalbitzer Equation 5b, for cumulative gas fractions between 10 and 90%
        Daa_MDD_b[0] = (1 / ((torch.pi**2) * tsec[0, 0])) * (
            (2 * torch.pi)
            - ((math.pi * math.pi / 3) * sumf_MDD[0])
            - (2 * math.pi) * (torch.sqrt(1 - (math.pi / 3) * sumf_MDD[0]))
        )
        Daa_MDD_b[1:] = (1 / ((math.pi**2) * diffti[1:])) * (
            -(math.pi * math.pi / 3) * diffFi[1:]
            - (2 * math.pi)
            * (
                torch.sqrt(1 - (math.pi / 3) * sumf_MDD[1:])
                - torch.sqrt(1 - (math.pi / 3) * sumf_MDD[0:-1])
            )
        )

        # Fechtig and Kalbitzer Equation 5c, for cumulative gas fractions greater than 90%
        Daa_MDD_c[1:] = (1 / (math.pi * math.pi * diffti[1:])) * (
            torch.log((1 - sumf_MDD[0:-1]) / (1 - sumf_MDD[1:]))
        )
        ...
        Daa_MDD = (
            use_a * Daa_MDD_a + torch.nan_to_num(use_b * Daa_MDD_b) + use_c * Daa_MDD_c
        )
        ...
        DR2_a[1:] = ((((sumf_MDD[1:] ** 2) - (sumf_MDD[0:-1]) ** 2)) * math.pi) / (
            4 * diffti[1:]
        )
        DR2_b[1:] = (4 / ((math.pi**2) * diffti[1:])) * np.log(
            (1 - sumf_MDD[0:-1]) / (1 - sumf_MDD[1:])
        )
