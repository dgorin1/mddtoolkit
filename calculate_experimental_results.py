...
def D0calc_MonteCarloErrors(expdata, geometry: str = "spherical"):
    ...
    if geometry == "spherical":
        ...
        DR2_a[0] = (Fi[0] ** 2 - 0.0**2) * math.pi / (36 * (cumtsec[0]))
        ...
        DR2_b[0] = (1 / ((math.pi**2) * tsec[0])) * (
            (2 * math.pi)
            - ((math.pi * math.pi / 3) * Fi[0])
            - (2 * math.pi) * (np.sqrt(1 - (math.pi / 3) * Fi[0]))
        )
        DR2_b[1:] = (1 / ((math.pi**2) * diffti)) * (
            -(math.pi * math.pi / 3) * diffFi
            - (2 * math.pi)
            * (
        ...
        DR2_c[1:] = (1 / (math.pi * math.pi * diffti)) * (
            np.log((1 - Fi[0:-1]) / (1 - Fi[1:]))
        )
    ...
    for m in range(1, nstep):  # For step of each experiment...
        for n in range(n_sim):
            MCdiffFi[m, n] = (
                MCFi[m, n] - MCFi[m - 1, n]
            )  # calculate the fraction released at each step
    ...
    if geometry == "spherical":
        ...
        for n in range(
            n_sim
        ):  # For each first step of an experiment, insert 0 for previous amount released
            MCDR2_a[0, n] = (
                ((MCFi[m, n]) ** 2 - (MCFi[m - 1, n]) ** 2)
                * math.pi
                / (36 * (diffti[m - 1]))
            )
        for m in range(
            1, nstep
        ):  # Calculate fechtig and kalbitzer equations for each fraction
            for n in range(n_sim):
                MCDR2_a[m, n] = (
                    ((MCFi[m, n]) ** 2 - (MCFi[m - 1, n]) ** 2)
                    * math.pi
                ...
                MCDR2_b[m, n] = (1 / ((math.pi**2) * diffti[m - 1])) * (
                    -(math.pi * math.pi / 3) * MCdiffFi[m, n]
                    - (2 * math.pi)
                    * (
                        np.sqrt(1 - (math.pi / 3) * MCFi[m, n])
                        - np.sqrt(1 - (math.pi / 3) * MCFi[m - 1, n])
                    )
                )
                MCDR2_c[m, n] = (1 / (math.pi * math.pi * diffti[m - 1])) * (
                    np.log((1 - MCFi[m - 1, n]) / (1 - MCFi[m, n]))
                )
        ...
        MCDR2 = (
            use_a_MC * MCDR2_a + np.nan_to_num(use_b_MC * MCDR2_b) + use_c_MC * MCDR2_c
        )
        ...
        for i in range(1, nstep):
            for j in range(n_sim):
                MCDR2_a[i, j] = (
                    ((MCFi[i, j]) ** 2 - (MCFi[i - 1, j]) ** 2)
                    * math.pi
                    / (4 * (tsec[i - 1]))
                )
                MCDR2_b[i, j] = (4 / ((math.pi**2) * diffti[i - 1])) * np.log(
                    (1 - MCFi[i - 1, j]) / (1 - MCFi[i, j])
                )
    ...
    return pd.DataFrame(
        {
            "Tplot": Tplot,
            "Fi": MCFimean.ravel(),
            "Fi uncertainty": delMCFi.ravel(),
            "Daa": DR2,
            "Daa uncertainty": MCDR2_uncert.ravel(),
            "ln(D/a^2)": np.log(DR2),
